...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write("Importing questionnaire...")
         ...
         # Build path to Knowledge Base/questionnaire.csv
        questionnaire_csv_path = os.path.join(settings.BASE_DIR.parent, 'Knowledge Base', 'questionnaire.csv')

        with open(questionnaire_csv_path, 'r') as f:
        ...
        # Build path to Knowledge Base/questionnaire.csv
        marking_grid_csv_path = os.path.join(settings.BASE_DIR.parent, 'Knowledge Base', 'marking_grid.csv')
        with open(marking_grid_csv_path, 'r') as f:
            reader = csv.DictReader(f)
            for row in reader:
                try:
        ...
        # Build path to Knowledge Base/questionnaire.csv
        recommendation_csv_path = os.path.join(settings.BASE_DIR.parent, 'Knowledge Base', 'recommendation.csv')
        with open(recommendation_csv_path, 'r') as f:
            reader = csv.DictReader(f)
            for row in reader:
                try:
                    q = Question.objects.get(id=int(row['Question Number']))
