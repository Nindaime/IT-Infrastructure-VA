...
import { mockQuestions } from '@/api/mockData'; // Assuming mockQuestions contains default question structure
...
function loadQuestionnaire() {
    ...
    if (qId && qId !== 'new') {
        ...
        const foundQ = {
            ...
            status: 'Draft',
            questions: mockQuestions.map(q => ({ ...q, options: [...q.options] })) // Deep copy options
        };
...
function removeOption(index) {
    if (newQuestion.value.options.length > 1) { // Ensure at least one option remains
...
function addQuestion() {
    ...
    if (!category.trim() || !text.trim() || cleanedOptions.length < 2) {
        setMessage('Please fill in question category, text, and provide at least two valid options.', 'error');
        return;
    }

    const newQ = {
        id: questionnaire.value.questions.length > 0 ? Math.max(...questionnaire.value.questions.map(q => q.id)) + 1 : 1,
...
watch(() => route.params.questionnaireId, () => {
    loadQuestionnaire();
});
</script>
...
<template>
    <div class="min-h-screen bg-gray-100 flex flex-col items-center p-6 font-sans">
        <div class="w-full max-w-4xl bg-white rounded-2xl shadow-xl p-8 md:p-10 my-8">
            ...
            <!-- Message Display -->
            <div v-if="message.text" :class="['p-4 mb-6 rounded-lg text-white font-medium',
            ...
            <section class="mb-8 p-6 bg-gray-50 rounded-lg border border-gray-200">
                ...
                <div class="mb-4">
                    <label for="questionnaire-name" class="block text-sm font-medium text-gray-700 mb-1">Questionnaire Name</label>
                    <input type="text" id="questionnaire-name" v-model="questionnaire.name"
                ...
                <div class="mb-4">
                    <label for="questionnaire-status" class="block text-sm font-medium text-gray-700 mb-1">Status</label>
                    <select id="questionnaire-status" v-model="questionnaire.status"
                        class="block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
            ...
            <section class="mb-8 p-6 bg-white rounded-lg border border-gray-200 shadow-sm">
                <h2 class="text-2xl font-semibold text-gray-800 mb-4">Existing Questions ({{ questionnaire.questions.length }})</h2>
                <div v-if="questionnaire.questions.length > 0" class="space-y-4 custom-scrollbar max-h-80 overflow-y-auto pr-2">
            ...
            <section class="mb-8 p-6 bg-blue-50 rounded-lg border border-blue-200">
                ...
                <div class="mb-4">
                    <label for="new-question-category" class="block text-sm font-medium text-gray-700 mb-1">Category</label>
                    <input type="text" id="new-question-category" v-model="newQuestion.category"
                        class="block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                        placeholder="e.g., Website Strength">
                </div>
                <div class="mb-4">
                    <label for="new-question-text" class="block text-sm font-medium text-gray-700 mb-1">Question Text</label>
                    <textarea id="new-question-text" v-model="newQuestion.text" rows="3"
                        class="block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                        placeholder="e.g., Is your website traffic encrypted using a valid SSL/TLS certificate?"></textarea>
                </div>
                <div class="mb-4">
                    ...
                    <div v-for="(option, index) in newQuestion.options" :key="index" class="flex items-center mb-2">
                        ...
                        <button v-if="newQuestion.options.length > 1" @click="removeOption(index)"
                            class="ml-2 p-2 rounded-full text-gray-500 hover:bg-gray-200 transition-colors">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                        </button>
                    </div>
                    <button @click="addOption" :disabled="!canAddOption"
                        class="mt-2 px-4 py-2 bg-gray-200 text-gray-700 rounded-md text-sm hover:bg-gray-300 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
                        Add Option
                    </button>
                </div>
